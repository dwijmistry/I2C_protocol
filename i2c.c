			/*i2c.c*/

#include"i2c_defines.h"
#include"i2c.h"
#include"delay.h"

void i2cStart(void){
	sda=1;
	scl=1;
	sda=0;
}

void i2cStop(void){
	scl=0;
	sda=0;
	scl=1;
	sda=1;
}

void i2cWrite(u8 dat){
	u8 i;
	for(i=0;i<8;i++){
		scl=0;
		sda=((dat>>(7-i))&1);	//sda=(dat&(0x80>>i))?1:0;
		scl=1;
	}
}

void i2cAck(void){
	scl=0;
	sda=1;
	scl=1;
	while(sda);
}

void i2cNoAck(void){
	scl=0;
	sda=1;
	scl=1;
}

void i2cMasterAck(void){
	scl=0;
	sda=0;
	scl=1;
	scl=0;
	sda=1;
}

u8 i2cRead(void){
	u8 i,buff=0;
	for(i=0;i<8;i++){
		scl=0;
		scl=1;
		if(sda)
			buff|=0x80>>i;
	}
	return buff;
}

void i2cReStart(void){
	scl=0;
	sda=1;
	scl=1;
	sda=0;
}

//Slave function protocols
void i2cDevWrite(u8 SlaveAddr, u8 buffAddr,u8 dat){
	i2cStart();
	i2cWrite(SlaveAddr & 0xfe);
	i2cAck();
	i2cWrite(buffAddr);
	i2cAck();
	i2cWrite(dat);
	i2cAck();
	i2cStop();
	delay_ms(10);
}

u8 i2cDevRead(u8 SlaveAddr, u8 buffAddr){
	u8 buff=0;
	i2cStart();
	i2cWrite(SlaveAddr & 0xfe);
	i2cAck();
	i2cWrite(buffAddr);
	i2cAck();
	i2cReStart();
	i2cWrite(SlaveAddr|=0x01);
	i2cAck();
	buff=i2cRead();
	i2cNoAck();
	i2cStop();
	return buff;
}

void i2cDevPageWrite(u8 *s8, u8 SlaveAddr, u8 buffStartAddr, u8 len){

	i2cStart();
	i2cWrite(SlaveAddr & 0xfe);
	i2cAck();
	i2cWrite(buffStartAddr);
	i2cAck();
	while(len--){
		i2cWrite(*s8++);
		i2cAck();
	}
	i2cStop();
}

void i2cDevPageRead(u8 *s8, u8 SlaveAddr, u8 buffStartAddr, u8 len){
	
	u8 i=0;
	i2cStart();
	i2cWrite(SlaveAddr & 0xfe);
	i2cAck();
	i2cWrite(buffStartAddr);
	i2cAck();
	i2cReStart();
	i2cWrite(SlaveAddr|=0x01);
	i2cAck();
	while(len-- >1){
		s8[i++]=i2cRead();
		i2cMasterAck();
	}
	s8[i]=i2cRead();
	i2cNoAck();
	i2cStop();
}
			/*end of i2c.c*/